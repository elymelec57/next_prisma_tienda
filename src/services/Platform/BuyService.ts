...
export class BuyService {
    ...
    async execute(form: any, pago: any, comprobante: any) {
        return await this.buyRepository.transaction(async (tx) => {
            // find restaurant by slug
            const restaurant = await this.buyRepository.findRestaurantBySlug(form.slug, tx)
            ...
            if (form.idpotencia) {
                const existing = await this.buyRepository.findPedidoByIdPotencia(form.idpotencia, restaurant.id, tx)
                if (existing) {
            ...
            if (conditions.length > 0) {
                cliente = await this.buyRepository.findClient(conditions, tx);
            }
            ...
            if (!cliente) {
                ...
                await this.buyRepository.updateClient(cliente.id, updateData, tx);
            }

            // 3. Generate order code + Create Pedido (Delivery orders don't require a mesa)
            const sucursalId = form.sucursalId !== 'main' ? Number(form.sucursalId) : null;
            const codigo = await getNextOrderCode(tx, restaurant.id, sucursalId);

            const pedido = await this.buyRepository.createPedido({
                data: {
                    codigo,
            ...
            if (pago) {
                if (comprobante) {
                    ...
                    await this.buyRepository.updateImage(image.id, String(payment.id), tx);
                } else {
                    payment = await this.buyRepository.createPayment({
                        data: {
                            monto: parseFloat(form.total),
